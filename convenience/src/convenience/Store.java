...
public class Store {
	 ...
	 * @param stock 입고된 상품의 갯구
	 */
	public void storeItem(int idx, String itemName, int itemPrice, int stock) {
		garage[idx] = new Item();
		garage[idx].name = itemName;
		garage[idx].price = itemPrice;
		garage[idx].stock = stock;
	}
	
	//어떤 아이템을 어디에 몇개 진열할 것인가
	/**
	 * 상품 진열
	 * @param itemidx 창고에 입고된 상품의 인덱스 번호
	 * @param sectionName 진열대 이름
	 * @param displayCount 진열할 상품의 수
	 */
	public void displayItem(int itemidx, String sectionName, int displayCount) {
		garage[itemidx].stock -= displayCount;
		if(sectionName == "A") {
			for (int i = 0; i < sectionA.items.length; i++) {
				if (sectionA.items[i] == null) {
					sectionA.items[i] = new Item();
					sectionA.items[i].name = garage[itemidx].name;
					sectionA.items[i].price = garage[itemidx].price;
...
//			sectionA.items[sectionIndex].name = garage[itemidx].name;
//			sectionA.items[sectionIndex].price = garage[itemidx].price;
//			sectionA.items[sectionIndex].stock = displayCount;
			...
			for (int i = 0; i < sectionB.items.length; i++) {
				if (sectionB.items[i] == null) {
					...
					sectionB.items[i].name = garage[itemidx].name;
					sectionB.items[i].price = garage[itemidx].price;
...
//			sectionB.items[sectionIndex].name = garage[itemidx].name;
//			sectionB.items[sectionIndex].price = garage[itemidx].price;
...
//		gs25.garage[3].stock = 16;
		...
		//진열대에 상품 추가
		//프링글스 오리지널을 A 진열대 2번 칸에 5개 진열한다.
